...
\end{figure} %TODO Caption einfügen
		...
		%TODO Deutsch machen, ich versteh nich was es bedeuten soll
		Die Semantische Web Technologie ist eine Erweiterung des heutigen Webs in dem die Informationen eine gut definierte, maschinell verarbeitete Bedeutung haben und es ermöglicht Maschinen und Menschen leicht zu Co-Operieren.\\
		Es ist gedacht das um das Web mit einem Netzwerk von adressierbare URI Informationen, diese sind so verlinkt und vorgestellt das es Maschinen leicht gemacht wird sowohl syntaktisch und semantisch darauf zuzugreifen.
		Für diesen Zweck sollten die Ressourcen mit Maschinen verständlichen Metadaten versehen das standardisiert ist durch Verwendung von Allgemeinem Vokabular und einer vordefinierten Semantik auch bekannt als "Ontologie"
		\\
			...
			\subsection{Distributed Hash Tables}
				...
				\subsubsection{CAN Algorithmus}
					\begin{figure}[h]
						...
						\label{fig:CAN}
					\end{figure}
					Der CAN Algorithmus ist der zweite hier vorgestellte Algorithmus für DHT bzw. strukturiertes p2p. Er ist dem Chord algorithmus sehr ähnlich hat jedoch zwei sehr große Unterschiede. Der erste ist, dass jede Komponente nur mit ihrem direkten Nachbarn kommuniziert wenn ein bestimmtes Element gesucht ist (durch die Strukturierung ist die Richtung bekannt), der zweite Unterschied ist die Lösung aus der eindimensionalen Kante eines Kreises hin zur n-dimensionalen Identifizierung von Daten. \\
